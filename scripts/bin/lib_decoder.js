...
export function encodeBase64(data, mimeType) {
  if (data == undefined) {
    return undefined;
  }
...
export function encodeBase64_padded(data) {
  if (data == undefined) {
    return undefined;
  }
  let _data = data;
  while (_data.length % 3 != 0) {
    _data += ' ';
  }
  const result = Buffer.from(_data).toString('base64');
  return result;
}
