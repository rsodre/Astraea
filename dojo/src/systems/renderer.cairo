...
pub impl AsterRendererImpl of AsterRendererTrait {
    ...
    // SVG builder
    //
    fn render_svg(token_props: @AsterProps) -> ByteArray {
        // let color_bg: ByteArray = token_props.palette.get_bg();
        // let colors: Span<ByteArray> = token_props.palette.get_colors();
        let palette_styles: ByteArray = token_props.palette.get_styles();
        //---------------------------
        // Build SVG
        //
        ...
        // ));
        result.append(@format!(
            "<style>.bl{{mix-blend-mode:screen;}}{}</style>",
                palette_styles,
        ));
        //
        // layers defs
        ...
        // Build text tags
        //
        let aster_count: usize = 1;
        let mut i: usize = 0;
        while i < aster_count {
            result.append(@"<g style=\"transform:scale(1)translate(0px, 0px)\">");
            result.append(@"<use href=\"#p4\" class=\"p1 r2\"/>");
            result.append(@"<use href=\"#p8\" class=\"p2 bl r1\"/>");
            result.append(@"<use href=\"#m12\" class=\"m1 r3\"/>");
            result.append(@"<use href=\"#m27\" class=\"m2 r4\"/>");
            result.append(@"</g>");
            i += 1;
        };
        //----------------
